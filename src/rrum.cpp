                      ...
                      unsigned int chain_length = 10000)
{
    ...
    for (unsigned int t = 0; t < chain_length; t++) {
        ...
        Rcpp::List output =
            parm_update(N, J, K, C, Y, Q, alpha, X, ss, gs, pis, vv, delta0);
...
//' @export
// [[Rcpp::export]]
Rcpp::List parm_update_MH(unsigned int N, unsigned int J, unsigned int K,
                          unsigned int C, const arma::mat Y, const arma::mat &Q,
                          arma::mat &alpha, arma::vec &pistar, arma::mat &rstar,
                          arma::vec &pi, const arma::vec vv,
                          const arma::vec &delta0, double delta,
                          const arma::mat &Amat)
{
    ...
    for (unsigned int j = 0; j < J; j++) {
        ...
        while ((pistar_new(j) <= 0) || (pistar_new(j) >= 1))
            pistar_new(j) = R::runif(a, b);
        for (unsigned int k = 0; k < task_ij.n_elem; k++) {
            kj = task_ij(k);
            c = rstar(j, kj) - delta;
            ...
            while ((rstar_new(j, kj) <= 0) || (rstar_new(j, kj) >= 1))
                rstar_new(j, kj) = R::runif(c, d);
        }
    }
    ...
    for (unsigned int c = 0; c < C; c++) {
        ...
        PENALTY = repmat(
            prod((rstar % (1 - Ac) % Q) + (Ac % Q) + (1 - Q), 1).t(), N, 1);
        like.col(c) =
            prod(PISTARrep % PENALTY % Y + (1 - PISTARrep % PENALTY) % (1 - Y),
                 1) *
            pi(c);
    }
    ...
    for (unsigned int i = 0; i < N; i++) {
        ...
        // Update alpha_ik
        alpha.row(i) =
            Amat.row(rgen::rmultinomial(like.row(i) / sum(like.row(i))));
        ...
        pistar_denom.row(i) =
            ((pistar %
              prod((rstar % (1 - Alpha) % Q) + (Q % Alpha) + (1 - Q), 1) % Yi) +
             ((1 -
               (pistar %
                prod((rstar % (1 - Alpha) % Q) + (Q % Alpha) + (1 - Q), 1))) %
              (1 - Yi)))
                .t();
        pistar_num.row(i) =
            ((pistar_new %
              prod((rstar_new % (1 - Alpha) % Q) + (Q % Alpha) + (1 - Q), 1) %
              Yi) +
             ((1 - (pistar_new %
                    prod((rstar_new % (1 - Alpha) % Q) + (Q % Alpha) + (1 - Q),
                         1))) %
              (1 - Yi)))
                .t();
    ...
    return Rcpp::List::create(
        ...
        Rcpp::Named("ar_rstar", ar_rstar), Rcpp::Named("ar_pistar", ar_pistar));
}
...
//' @export
// [[Rcpp::export]]
Rcpp::List rRUM_MH(const arma::mat &Y, const arma::mat &Q, arma::mat &Amat,
                   double delta, unsigned int chain_length = 10000)
{
    ...
    for (unsigned int t = 0; t < chain_length; t++) {
        // updata X,alpha,pi,s,g,pistar,rstar
        Rcpp::List output =
            parm_update_MH(N, J, K, C, Y, Q, alpha, pistars, rstars, pis, vv,
                           delta0, delta, Amat);
